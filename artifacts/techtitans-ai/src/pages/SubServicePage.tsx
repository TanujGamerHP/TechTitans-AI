...
export default function SubServicePage() {
  ...
  const filteredProjects = projects.filter(
    (p) => p.serviceId === params.serviceId && p.subServiceId === params.subServiceId
  );

  // Fallback: show all projects for the service if no sub-service match
  const allServiceProjects = projects.filter((p) => p.serviceId === params.serviceId);
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-16 pb-24">
        ...
        {displayProjects.length === 0 ? (
          <div className="glass-card rounded-3xl p-16 text-center">
            <p className="text-foreground-muted text-lg mb-2">Projects coming soon.</p>
            <p className="text-foreground-muted text-sm">We're currently completing work in this area — check back soon or explore our full portfolio.</p>
            <button
              onClick={() => navigate("/portfolio")}
              className="mt-6 inline-flex items-center gap-2 text-primary hover:underline text-sm"
            >
